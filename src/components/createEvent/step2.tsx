...
const Step2 = () => {
    ...
    const addNewRole = () => {
        if (newRole.trim()) {
            const newRoleObj = { key: newRole.toLowerCase(), label: newRole };

            // Check for duplicate roles
            if (!eventData.roles.some((role) => role.key === newRoleObj.key)) {
                setEventData({
                    ...eventData,
                    roles: [...eventData.roles, newRoleObj],
                });
            }
            setNewRole('');
        }
    ...
    const {
        ...
        defaultValues: {
            ...
            role: eventData.roles.map((role) => role.key).join(', '),
